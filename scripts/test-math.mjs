...
// 前置式：不能把表达式塞进下标
check('lim f(x) 前置式保持并列',
  renderMath('lim f(x)/g(x) = A'),
  'lim f(x)/g(x) = A');

// 裸 lim
check('独立的 lim 不加下标', renderMath('极限 lim 的值'), '极限 lim 的值');

// 括号为空时不渲染空下标
check('lim() 空括号保持原样', renderMath('lim() 写法有误'), 'lim() 写法有误');
...
console.log('  输出: ' + h);
check('管线里 math-lim 标记被正确还原（不是 &lt;span）', h.includes('<span class="math-lim">'), true);
check('管线里保留了白名单 code 标签', h.includes('<code>sin x / x</code>'), true);

// 安全性：内容里混入恶意脚本必须被转义
const evil = richInline('lim(x→0) <script>alert(1)</script> <img src=x onerror=alert(1)>');
check('恶意 script 被转义', evil.includes('<script>'), false);
check('恶意 img 被转义', evil.includes('<img'), false);
check('尖括号被转义成实体', evil.includes('&lt;script&gt;'), true);
...
check('没有残留的抽象极限写法', leftoverParen, 0);
check('没有残留未转换的 lim<sub>', leftoverSub, 0);
check('全部 span 标签配平', unbalanced, 0);
